...
function assertLineIndexMatchesRebuild(
...
describe("Editor Use Cases", () => {
  ...
  describe("Selection Operations", () => {
    it("should set cursor position", () => {
      ...
      store.dispatch(
        DocumentActions.setSelection([{ anchor: byteOffset(5), head: byteOffset(5) }]),
      );
    ...
    it("should set text selection range", () => {
      ...
      // Select "World"
      store.dispatch(
        DocumentActions.setSelection([{ anchor: byteOffset(6), head: byteOffset(11) }]),
      );
              ...
              changes.map((change) =>
                change.type === "insert"
                  ...
                  : {
                      ...
                      start: byteOffset(change.start),
                      length: byteLength(change.length),
                    },
              ),
            ),
          );
