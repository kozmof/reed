...
describe('canUndo', () => {
  ...
  it('should work with HistoryState directly', () => {
    const historyState: HistoryState = {
      undoStack: [
        {
          changes: [{ type: 'insert', position: 0, text: 'a' }],
  ...
  it('should return true after multiple edits', () => {
    ...
    state = documentReducer(state, DocumentActions.insert(1, 'b'));
    state = documentReducer(state, DocumentActions.insert(2, 'c'));
    expect(canUndo(state)).toBe(true);
  });
});
  ...
  it('should work with HistoryState directly', () => {
    const historyState: HistoryState = {
      undoStack: [
        ...
        {
          changes: [{ type: 'insert', position: 1, text: 'b' }],
...
describe('HISTORY_CLEAR action', () => {
  it('should clear both undo and redo stacks', () => {
    let state = createInitialState();
    state = documentReducer(state, DocumentActions.insert(0, 'Hello'));
    state = documentReducer(state, DocumentActions.insert(5, ' World'));
    state = documentReducer(state, DocumentActions.undo());
...
describe('History helpers integration', () => {
  ...
  it('should clear redo stack on new edit', () => {
    ...
    state = documentReducer(state, DocumentActions.insert(1, 'c'));
