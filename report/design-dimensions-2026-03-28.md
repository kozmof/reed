...
**`fixInsertWithPath`** (`rb-tree.ts:282-304`) walks the insertion path _from leaf to root_ (collected during BST descent, then reversed). This replaces the classic O(n) `fixInsert` with an O(log n) bounded upward walk:
...
`offsetDelta` in `DirtyLineRange` is the byte shift to apply to lines _within that range_. It is not a global offset — multiple ranges have independent deltas, and the sweep-line algorithm accumulates them:
...
**Version semantics:** Background reconciliation does _not_ increment `state.version` (invisible to users). `reconcileNow()` does (user-initiated).
...
The reducer calls `getDeleteBoundaryContext` _before_ the delete phase to snapshot `prevChar`/`nextChar` from the piece table. This two-character lookahead is the minimum context needed to detect all CRLF-crossing cases without reading the full document.
...
The system's explicit disclaimer in `cost.ts:14-20`: _"Cost labels are documentation annotations, not runtime contracts."_
...
**Buffer model extension:** `BufferType` extended from `'original' | 'add'` to `'original' | 'add' | 'chunk'`. A third `ChunkBufferRef` variant joins the `BufferReference` discriminated union. Each `'chunk'` piece carries a `chunkIndex: number` identifying its entry in `PieceTableState.chunkMap: ReadonlyMap<number, Uint8Array>`. `start` remains the offset _within the chunk_, preserving the existing buffer-access contract in `getBufferSlice`. Non-chunk pieces carry `chunkIndex: -1`.
